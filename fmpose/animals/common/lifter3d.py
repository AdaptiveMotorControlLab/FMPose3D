...
joint_names = ['snout', 'Right_Ear', 'Left_Ear', 'Shoulder_Center', 'Right_Paw', 
               'Right_Wrist', 'Right_Elbow', 'Right_Shoulder', 'Left_Paw', 'Left_Wrist', 
               'Left_Elbow', 'Left_Shoulder', 'Body_Center', 'Hip_Center', 'Right_Foot', 
               'Right_Ankle', 'Right_Knee', 'Left_Foot', 'Left_Ankle', 'Left_Knee', 
               'Tail_Tip', 'Tail_Middle', 'Tail_Root']
...
# 关节连接（Skeleton）
skeleton = [
    ['snout', 'Right_Ear'], ['snout', 'Left_Ear'],
    ['Shoulder_Center', 'Right_Shoulder'], ['Right_Shoulder', 'Right_Elbow'], 
    ['Right_Elbow', 'Right_Wrist'], ['Right_Wrist', 'Right_Paw'],
    ['Shoulder_Center', 'Left_Shoulder'], ['Left_Shoulder', 'Left_Elbow'], 
    ['Left_Elbow', 'Left_Wrist'], ['Left_Wrist', 'Left_Paw'],
    ['Shoulder_Center', 'Body_Center'], ['Body_Center', 'Hip_Center'],
    ['Hip_Center', 'Right_Knee'], ['Right_Knee', 'Right_Ankle'], ['Right_Ankle', 'Right_Foot'],
    ['Hip_Center', 'Left_Knee'], ['Left_Knee', 'Left_Ankle'], ['Left_Ankle', 'Left_Foot'],
    ['Tail_Root', 'Tail_Middle'], ['Tail_Middle', 'Tail_Tip'],['Hip_Center','Tail_Root']
]
...
def compute_reprojection_errors(keypoints_2d, reprojected_2d):
    # 计算欧几里得距离
    errors = np.linalg.norm(keypoints_2d - reprojected_2d, axis=-1)  # 形状: (num_frames, num_keypoints)

    # 计算整体误差（所有帧、所有关键点的平均误差）
    total_error = np.mean(errors)

    # 计算逐个关键点的误差（对 num_keypoints 维度取均值）
    per_keypoint_error = np.mean(errors, axis=0)  # 形状: (num_keypoints,)
...
def compute_relative_errors(keypoints_2d, reprojected_2d):
    # 计算欧几里得误差 (num_frames, num_keypoints)
    errors = np.linalg.norm(keypoints_2d - reprojected_2d, axis=-1)

    # 计算总体误差
    total_error = np.mean(errors)
    
    # 计算逐个关键点的误差 (num_keypoints,)
    per_keypoint_error = np.mean(errors, axis=0)

    # 计算关键点两两之间的欧几里得距离 (num_frames, num_keypoints, num_keypoints)
    pairwise_dists = np.linalg.norm(keypoints_2d[:, :, None, :] - keypoints_2d[:, None, :, :], axis=-1)

    # 计算关键点间距的平均值 (num_frames,)
    avg_keypoint_distance = np.mean(pairwise_dists, axis=(1, 2))  # 按帧平均

    # 计算总相对误差
    relative_error = total_error / np.mean(avg_keypoint_distance)

    # 计算逐个关键点的相对误差 (num_keypoints,)
...
def normalize_points(points_3d):
    """ 归一化 3D 点到 [-1, 1] """
    min_vals = points_3d.min(axis=(0, 1), keepdims=True)  # 计算所有帧的最小值
    max_vals = points_3d.max(axis=(0, 1), keepdims=True)  # 计算所有帧的最大值
    points_3d_normalized = (points_3d - min_vals) / (max_vals - min_vals) * 2 - 1  # 归一化到 [-1, 1]
...
def plot_3d_keypoints_and_save_video(points_3d,output_video_path):
    ...
    out = cv2.VideoWriter(output_video_path, fourcc, 30.0, (1024, 768))  # Adjust the frame size if needed
    ...
    for frame in range(num_frames_to_save):
        ...
        for bone in skeleton:
            if bone[0] in joint_names and bone[1] in joint_names:
                ...
                ax.plot(x_vals, y_vals, z_vals, color='black', linewidth=2, alpha=0.8)  # 画连线
...
def triangulate_3d_batch(points_2d_batch, cameras):
    ...
    proj_matrices = np.array([
        cam["intrinsic_matrix"] @ np.hstack((cam["R"], cam["T"]))
        for cam in cameras
    ])  # numpy array (6, 3, 4)
    ...
    for i in range(num_cams):
        ...
        undistorted = undistorted.reshape(num_frames, num_joints, 2)  # 变回 (num_frames, 23, 2)
        print("undistorted shape:", undistorted.shape)
        print("ones shape:", np.ones((num_frames, num_joints, 1)).shape)  # 预期 (num_frames, num_joints, 1)
        # undistorted = (K @ np.hstack([undistorted, np.ones((num_frames, num_joints, 1))]).T).T[:, :, :2]
        undistorted = np.concatenate([undistorted, np.ones((*undistorted.shape[:-1], 1))], axis=-1)  # (50, 23, 3)
        undistorted = (K @ undistorted[..., None])[..., 0]  # 批量矩阵乘法
        points_2d_undistorted[i] = undistorted[..., :2]  # 存储去畸变结果
 ...
 # **3. Construct matrix A for triangulation**
    ...
    A = np.stack([x[..., None] * P3 - P1, y[..., None] * P3 - P2], axis=-2)  # (6, num_frames, 23, 2, 4)
...
def triangulate_3d(points_2d, cameras): 
    ...
    for i, cam in enumerate(cameras):
        ...
        undistorted = cv2.undistortPoints(points_2d[i].reshape(-1, 1, 2), K, dist, None, None).reshape(-1, 2)
    ...
    for j in range(num_points):  # 遍历所有 2D 点
        A = []
        for i in range(len(proj_matrices)):  # 遍历所有相机
            P = proj_matrices[i]  # 当前相机的投影矩阵
            x, y = points_2d_undistorted[i][j]  # 第 i 个相机的第 j 个 2D 点

            # 生成线性方程 Ax = 0
            A.append(x * P[2, :] - P[0, :])  # 第一行约束
            A.append(y * P[2, :] - P[1, :])  # 第二行约束

        A = np.array(A)
        _, _, Vh = np.linalg.svd(A)  # SVD 求解
        X_hom = Vh[-1]  # 最小奇异值对应的解
        X = X_hom[:3] / X_hom[3]  # 归一化
        points_3d[j] = X  # 存储 3D 点

    # print("3D points and the 2D on camera 0",points_3d,points_2d_undistorted[0])
    return points_3d  # 返回所有 3D 点

def triangulate_3d_confi(points_2d, cameras): 
    # points_2d: list of 6 in (23, 3), 最后一维为置信度
    ...
    for i, cam in enumerate(cameras):
        ...
        # Undistortion
        undistorted = cv2.undistortPoints(points_2d[i][:, :2].reshape(-1, 1, 2), K, dist, None, None).reshape(-1, 2)
        undistorted = (K @ np.hstack([undistorted, np.ones((undistorted.shape[0], 1))]).T).T[:, :2]
        points_2d_undistorted.append(undistorted)
        
        # Collect confidences
        confidences.append(points_2d[i][:, 2])  # 提取置信度

    num_points = points_2d_undistorted[0].shape[0]
    points_3d = np.zeros((num_points, 4))  # 最后一维为置信度
    ...
    for j in range(num_points):  # 遍历所有 2D 点
        ...
        for i in range(len(proj_matrices)):  # 遍历所有相机
            P = proj_matrices[i]  # 当前相机的投影矩阵
            x, y = points_2d_undistorted[i][j]  # 第 i 个相机的第 j 个 2D 点
            conf = confidences[i][j]  # 当前点的置信度

            if conf > 0:  # 仅使用置信度大于 0 的点
                # 生成线性方程 Ax = 0
                A.append(conf * (x * P[2, :] - P[0, :]))  # 第一行约束
                A.append(conf * (y * P[2, :] - P[1, :]))  # 第二行约束
                point_confidences.append(conf)

        if len(A) > 0:  # 至少有一个有效视角
            A = np.array(A)
            _, _, Vh = np.linalg.svd(A)  # SVD 求解
            X_hom = Vh[-1]  # 最小奇异值对应的解
            X = X_hom[:3] / X_hom[3]  # 归一化
            points_3d[j, :3] = X  # 存储 3D 点

            # 计算 3D 点的置信度：所有视角置信度的平均值
            points_3d[j, 3] = np.mean(point_confidences)
        else:
            points_3d[j, 3] = 0  # 如果没有有效视角，置信度为 0

    return points_3d  # 返回所有 3D 点，形状为 (num_points, 4)

# def project_3d_to_2d(points_3d, camera):
#     K, dist, R, T = camera["intrinsic_matrix"], camera["distortion_coeffs"], camera["R"], camera["T"]
    
#     points_3d = np.asarray(points_3d, dtype=np.float32)  # 强制转换为 float32
...
def visualize_2d_on_video(video_path, frame_number, original_keypoints, reprojected_keypoints, output_path):
    ...
    if not ret:
        print(f"无法读取视频 {video_path} 中的第 {frame_number} 帧")
        cap.release()
        return
    ...
    for i, point in enumerate(original_keypoints):
        if not np.isnan(point).any():
            ...
            if x < 0 or y < 0 or x >= w or y >= h:
                continue
            plt.scatter(x,y,marker= '+',color = colors[i],s =15,linewidths=1,label = f"keypoints{i}")
...
def main():
    yaml_files = sorted(glob.glob('/workspace/MTFpose/data/Arber_data_noeva/076_02_221114_start0/calibration/*.yaml'))
    h5_files = sorted(glob.glob('/workspace/MTFpose/data/Arber_data_noeva/076_02_221114_start0/pose2d_dlc/*.h5'))
    video_files = sorted(glob.glob('/workspace/MTFpose/data/Arber_data_noeva/076_02_221114_start0/video_dlc/*.mp4'))
    ...
    print("list: yaml ",len(yaml_files),"h5",len(h5_files),"video ",len(video_files),"cameras",len(cameras),"keypoints",len(keypoints_2d))
    ...
    for frame in tqdm(range(total_num_frames), desc="Processing frames", unit="frame"): # loop in frames
    ...
    output_npy_path = "/workspace/MTFpose/data/Arber_data_noeva/076_02_221114_start0/triangulated_3d.npy"
    np.save(output_npy_path,points_3d)
    # print("points_3d_fragment from 3d lift and stack, shape",points_3d.shape) 
    
    #     if frame==frame_number:# 可视化第n帧并保存为PNG文件
    #         for i in range(len(cameras)):
    ...
    for i in range(len(cameras)):
        ...
        points_3d_array = np.array(points_3d_list).reshape(total_num_frames,-1,3) # with confidence
