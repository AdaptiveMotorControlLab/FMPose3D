...
class Human36mDataset(MocapDataset):
    def __init__(self, path, opt, remove_static_joints=True):
        ...
        data = np.load(path, allow_pickle=True)[
            "positions_3d"
        ].item()  # type(data)= dict,len=7, {'S1':{'action':},'S5':[],...,'S11':[]} ，len(actions)>=15,

        self._data = (
            {}
        )  # {'S1':{'action1':{'positions': positions,'cameras': self._cameras[subject]}},}
        ...
        ):  # total 32keypoints, removed 15joints, remain 17 joints
            self.remove_joints(
