    ...
    def __init__(self, cfg, path, split, cam_names,
        ...
        for sub_idx, subject_name in enumerate(self.subject_list): # iterate on subject 
            ...
            for cam in cam_names:
                yaml_files.extend(sorted(glob.glob(os.path.join(subject_folder, f'calibration/*{cam}*.yaml'))))
            ...
            for cam in cam_names:
                ...
                h5_files.extend(sorted(glob.glob(os.path.join(subject_folder,f'pose2d_dlc/*{cam}*.h5')))) # for rtmpose model
                
            keypoints_2d = [load_h5_keypoints(h5) for h5 in h5_files]   # （num_cameras,num_frames,23,3) # for rtmpose model
            ...
            for idx in tqdm(range(self.start_frame,real_end_frame,sampling_gap)): # get temporal video fragment of 2D and 3D keypoints
                ...
                points_3d_fragment = np.load(os.path.join(subject_folder,'triangulated_3d.npy'))[left_frame_id:right_frame_id,:,:3]/norm_rate # load from prepared .npy and apply norm_rate
                ...
                points_3d =  np.zeros((self.t_length, self.joint_num, 3, len(self.cam_names)))  # initialize 3D keypints, from T,K,3 to T,K,3,N
                ...
                for cam_idx, cam in enumerate(cam_names):
                    ...
                    points_3d_cam = np.dot(points_3d_world,cameras[cam_idx]['R'].T) + cameras[cam_idx]['T'].T  
                    
                    points_3d[:,:,:,cam_idx] = np.reshape(points_3d_cam,(self.t_length,self.joint_num,3)) #T,K,3,N
                
                # get relative 3D pose 
                points_3d_root = copy.deepcopy(points_3d[:,self.root_index:self.root_index+1,:,:])
                ...
                # normalize 2D pose 
                points_2d_np = normalize_screen_coordinates(copy.deepcopy(points_2d_np),self.img_W,self.img_H) # N,T,K,2
                
                # get fake vis3d
                points_vis3D = np.ones((self.t_length, self.joint_num, 1))
                
                self.pose_3D_list.append(rela_points_3d)
                self.pose_2D_list.append(np.nan_to_num(points_2d_np.transpose(1,2,3,0)))    # transpose to T,K,2,N
                self.vid2D_list.append(points_2d_vis_np.transpose(1,2,3,0))  # Transpose to T,K,1,N, move N to the end
...
if __name__ == '__main__':
    ...
    causal_shift = 0
    train_dataset = ArberDataset(cfg, cfg.ARBER_DATA.ROOT_DIR, 'Train', cam_names, pad, root_index = root_index, 
                             use_2D_gt = use_2d_gt, joint_num = 23, sampling_gap = 60,
                             img_W = img_W, img_H = img_H, arg_views = 0, 
                            resize_2D_scale = cfg.ARBER_DATA.RESIZE_SCALE)
    
    pose_3D, pose_root, pose_2D, vid_3D, rotation, sample_info = train_dataset.getitem(2)
    print("output at item 250, pose_3D",pose_3D.shape, "pose_root",pose_root.shape,"pose_2D",pose_2D.shape,"vid_3D",vid_3D.shape,"rotation",rotation.shape,"sample_info",sample_info)
