...
def show_frame():
  ...
  for i_data, data in enumerate(tqdm(dataloader, 0)):
    ...
    # input_2D_nonflip = torch.randn_like(input_2D_nonflip) 
    # only add noise to the left leg of the input_2D_nonflip;
    # input_2D_nonflip[:, :, 4, :] = input_2D_nonflip[:, :, 4, :] + torch.randn_like(input_2D_nonflip[:, :, 4, :]) * 0.5
    # input_2D_nonflip[:, :, 5, :] = input_2D_nonflip[:, :, 5, :] + torch.randn_like(input_2D_nonflip[:, :, 5, :]) * 0.5
    # input_2D_nonflip[:, :, 6, :] = torch.randn_like(input_2D_nonflip[:, :, 6, :]) * 0.5
    # input_2D_nonflip[:, :, 5, :] = 0
    # input_2D_nonflip[:, :, 6, :] = 0
    
    # input_2D_nonflip[:, :, 5, :] = torch.randn_like(input_2D_nonflip[:, :, 5, :])
    # input_2D_nonflip[:, :, 6, :] = torch.randn_like(input_2D_nonflip[:, :, 6, :])
    
    input_2D_nonflip[:, :, 6, :] += torch.randn_like(input_2D_nonflip[:, :, 6, :])*0.2
    # input_2D_nonflip = input_2D_nonflip + torch.randn_like(input_2D_nonflip)
    # input_2D_nonflip = torch.zeros_like(input_2D_nonflip)
    ...
    if refine:
        ...
        refined_output[:, :, 0, :] = 0
        output_mgcn = refined_output.clone()
    ...
    #     # return y_local
        ...
        # Simple Euler sampler for CFM at test time with Classifier-Free Guidance support
    def euler_sample(x2d, y_local, steps_local, model_3d, guidance_scale=1.0):
        list_v_s = []
        dt = 1.0 / steps_local
        for s in range(steps_local):
            ...
            if guidance_scale != 1.0:
                ...
                # Predict with conditioning (original x2d)
                v_cond = model_3d(x2d, y_local, t_s)
                
                # Predict without conditioning (zeros)
                x2d_uncond = torch.zeros_like(x2d)
                v_uncond = model_3d(x2d_uncond, y_local, t_s)
                
                # Combine predictions: v = v_uncond + guidance_scale * (v_cond - v_uncond)
                v_s = v_uncond + guidance_scale * (v_cond - v_uncond)
            else:
    ...
    for s_keep in eval_steps:
        ...
        for i in range(args.hypothesis_num):
            ...
            y_s, list_v_s = euler_sample(input_2D_nonflip, y, s_keep, model_FMPose, guidance_scale)
        ...
        # output_fmpose = aggregate_hypothesis(list_hypothesis)
        output_fmpose = y_s
        # output_fmpose = aggregate_hypothesis_camera_weight(list_hypothesis, batch_cam, input_2D_nonflip, gt_3D, args.topk)
      
    # opt_num = 5
    # for s in range(opt_num):
      # output_fmpose, _ = euler_sample(input_2D_nonflip, output_fmpose, 3, model_FMPose, guidance_scale) 
           
    output_fmpose[:, :, 0, :] = 0
    
    fmpose_mpjpe = (mpjpe_cal(output_fmpose, out_target) * 1000).item()
    mgcn_refined_mpjpe = (mpjpe_cal(output_mgcn, out_target) * 1000).item()
    ...
    # ax1.set_title("GT_3D")
    mgcn_np = output_mgcn[0, 0].cpu().detach().numpy()
    show3Dpose( mgcn_np, ax1, color, world = False, linewidth=linewidth)
