...
from model.graph_frames_rat import Graph
...
class Model(nn.Module):
    def __init__(self, args):
        ...
        ## GCN - Use Rat7M skeleton structure
        self.graph = Graph('rat7m', 'spatial', pad=1)
...
if __name__ == "__main__":
    ...
    args.token_dim = 256
    args.layers = 5
    args.n_joints = 20  # Rat7M has 20 joints
    ...
    # Test with Rat7M data shape
    x = torch.randn(2, 1, 20, 2, device=device)  # (B, F, J=20, 2)
    y_t = torch.randn(2, 1, 20, 3, device=device)  # (B, F, J=20, 3)
    t = torch.randn(2, 1, 1, 1, device=device)
    v = model(x, y_t, t)
    print(f"Input shape: {x.shape}")
    print(f"Output shape: {v.shape}")
    print("✓ Model test passed for Rat7M!")
