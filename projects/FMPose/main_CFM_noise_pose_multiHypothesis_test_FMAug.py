...
def uncertainty_aware_aggregate_hypothesis(list_hypothesis, k=None):
    ...
    # threshold factor k; can be provided by caller
    if k is None:
        k = getattr(args, 'ua_k', 0.9)
...
def aggregate_hypothesis_camera(list_hypothesis, batch_cam, input_2D, gt_3D):
    """
...
def test_multi_hypothesis(args, actions, dataLoader, model, optimizer=None, epoch=None, hypothesis_num=None, steps=None):
    ...
    for i, data in enumerate(tqdm(dataLoader, 0)):
        ...
        def euler_sample_flowaug(x2d, x2d_flip, y_local, steps):
            dt = 1.0 / steps

            for s in range(steps):
                y_flip = y_local.clone()
                t_s = torch.full((y_local.size(0), 1, 1, 1), s * dt, device=gt_3D.device, dtype=gt_3D.dtype)
                ...
                v_s_flip = model_3d(x2d_flip, y_flip, t_s)
                y_flip = y_flip + dt * v_s_flip
                # flip back
                y_flip[:, :, :, 0] *= -1
                y_flip[:, :, args.joints_left + args.joints_right, :] = y_flip[:, :, args.joints_right + args.joints_left, :]
        
                y_local = (y_local + y_flip) / 2
        ...
        for s_keep in eval_steps:
            ...
            # Generate all N noise samples at once: (B*N, F, J, 3)
            ...
            if args.test_augmentation_FlowAug:
                input_2D_flip_expanded = input_2D_flip.repeat(N, 1, 1, 1)
                y_s_batch = euler_sample_flowaug(input_2D_nonflip_expanded, input_2D_flip_expanded, y_batch, s_keep)
            else:
            ...
            # Convert to list of hypotheses: each element is (B, 1, J, 3)
            for i in range(N):
