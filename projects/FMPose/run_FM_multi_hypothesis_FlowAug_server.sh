#Test CFM with multi-hypothesis and FlowAug
layers=5
batch_size=1024
epochs=100
sh_file='run_FM_multi_hypothesis_FlowAug_server.sh'
...
exp_temp=0.005
FlowAug=False
folder_name=Cam_s${eval_multi_steps}_Top${topk}_${mode}_temp${exp_temp}_h${num_hypothesis_list}_test_results_$(date +%Y%m%d_%H%M%S)
job_name=fm-multi-hyp-flowaugx7
...
PYTHON_CMD="cd /data/ti/Ti_workspace/projects/FMPose && python3 main_CFM_noise_pose_multiHypothesis_test_FMAug.py \
--reload \
--topk ${topk} \
--exp_temp ${exp_temp} \
...
--test_augmentation_FlowAug ${FlowAug} \
