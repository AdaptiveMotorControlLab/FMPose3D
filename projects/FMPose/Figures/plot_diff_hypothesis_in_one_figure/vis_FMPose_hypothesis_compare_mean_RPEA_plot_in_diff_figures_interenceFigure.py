...
def show3Dpose_GT(channels, ax, world = True, linewidth = 2.5): #red
...
def show_frame():
  ...
  for i_data, data in enumerate(tqdm(dataloader, 0)):
    ...
    for s_keep in eval_steps:
        ...
        output_3D_mean = aggregate_hypothesis(list_hypothesis)
        output_3D_RPEA = aggregate_hypothesis_camera_weight(list_hypothesis, batch_cam, input_2D_nonflip, gt_3D, args.topk)
    ...
    if delta < 10 or loss_mean > 60:
      continue
    print(f"loss_mean: {loss_mean.item():.2f}, loss_RPEA: {loss_RPEA.item():.2f}, delta: {delta.item():.2f}")
    ...
    plt.savefig(pose_2d_path, dpi=dpi_number, format='jpg', bbox_inches='tight', transparent=False)
    plt.close(fig1)
    print(f"Saved 2D pose: {pose_2d_path}")
    ...
    if 'list_hypothesis_last' in locals() and 'output_3D_mean' in locals():
      ...
      for idx, hypo in enumerate(list_hypothesis_last):
        ...
        if num_h > 1:
          ...
          shade = 0.6
        show3Dpose(pose_np, ax_hypo, color=(shade, shade, shade), world=False, linewidth=2.5)
        # Save individual hypothesis
        hypo_path = os.path.join(hypothesis_folder, f'hypothesis_{idx:02d}.jpg')
        plt.savefig(hypo_path, dpi=dpi_number, format='jpg', bbox_inches='tight', transparent=False)
        plt.close(fig_hypo)
      ...
      all_hypo_path = os.path.join(path, action[0] + '_idx_' + str(i_data) + '_all_hypotheses.jpg')
      plt.savefig(all_hypo_path, dpi=dpi_number, format='jpg', bbox_inches='tight', transparent=False)
      ...
      mean_path = os.path.join(path, action[0] + '_idx_' + str(i_data) + '_delta_' + f'{delta.item():.1f}' + '_mean.jpg')
      plt.savefig(mean_path, dpi=dpi_number, format='jpg', bbox_inches='tight', transparent=False)
      plt.close(fig_mean)
      ...
      for idx, hypo in enumerate(list_hypothesis_last):
        ...
        if num_h > 1:
          ...
          shade = 0.6
        show3Dpose(pose_np, ax_rpea, color=(shade, shade, shade), world=False, linewidth=repa_linewidth)
      
      # Plot RPEA aggregated result in orange
      agg_rpea_vis = output_3D_RPEA.clone()
      agg_rpea_vis[:, :, 0, :] = 0
      agg_rpea_np = agg_rpea_vis[0, 0].cpu().detach().numpy()
      show3Dpose(agg_rpea_np, ax_rpea, color=(255/255, 140/255, 0/255), world=False, linewidth=repa_linewidth)  # Orange color
      
      # Save RPEA aggregation figure
      rpea_path = os.path.join(path, action[0] + '_idx_' + str(i_data) + '_delta_' + f'{delta.item():.1f}' + '_rpea.jpg')
      plt.savefig(rpea_path, dpi=dpi_number, format='jpg', bbox_inches='tight', transparent=False)
      plt.close(fig_rpea)
