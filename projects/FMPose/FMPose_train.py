...
def step(
    split, args, actions, dataLoader, model, optimizer=None, epoch=None, steps=None
):
    ...
    for i, data in enumerate(tqdm(dataLoader, 0)):
        ...
        if split == "train":
            ...
            action_error_sum = test_calculation(
                output_3D, out_target, action, action_error_sum, args.dataset, subject
            )
...
if __name__ == "__main__":
    ...
    if args.create_file:
        ...
        if args.train == False:
            ...
            args.checkpoint = os.path.join(
                args.folder_dir, "test_results_" + args.create_time
            )
    ...
    for epoch in range(1, args.nepoch):
        ...
        if args.train:
            ...
            step_strs = [
                f"{s}_p1: {p1_per_step[s]:.4f}, {s}_p2: {p2_per_step[s]:.4f}"
                for s in steps_sorted
            ]
            print(
                "e: %d, lr: %.7f, loss: %.4f, p1: %.4f, p2: %.4f | %s"
                % (epoch, lr, loss, p1, p2, " | ".join(step_strs))
            )
            logging.info(
                "epoch: %d, lr: %.7f, loss: %.4f, p1: %.4f, p2: %.4f | %s"
                % (epoch, lr, loss, p1, p2, " | ".join(step_strs))
            )
