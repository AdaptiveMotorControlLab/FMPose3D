...
class Model(nn.Module):
    def __init__(self, args):
        ...
        # intrinsic camera embedding
        self.cam_embed_dim = 32
        self.camera_embed = nn.Sequential(
            nn.Linear(9, self.cam_embed_dim),
            nn.GELU(),
            nn.Linear(self.cam_embed_dim, self.cam_embed_dim)
        )

        # encoder maps concatenated [x2d(2), y_t(3), t_emb(self.t_embed_dim), cam_emb(self.cam_embed_dim)] to args.channel
        self.encoder = encoder(2 + 3 + self.t_embed_dim + self.cam_embed_dim, args.channel//2, args.channel)
        #  
    ...
    def forward(self, x, intrinsic_cam, y_t, t):
        # x: (B,F,J,2)  intrinsic_cam: (B,9); y_t: (B,F,J,3)  t: (B,1,1,1) or (B,F,1,1)
        b, f, j, _ = x.shape
        # build time embedding
        t_emb = self.time_embed(t) # (B,F,t_dim)
        t_emb = t_emb.unsqueeze(2).expand(b, f, j, self.t_embed_dim).contiguous()  # (B,F,J,t_dim)

        cam_emb = self.camera_embed(intrinsic_cam.to(device=x.device, dtype=x.dtype))  # (B,cam_dim)
        cam_emb = cam_emb.unsqueeze(1).unsqueeze(2).expand(b, f, j, self.cam_embed_dim).contiguous()  # (B,F,J,cam_dim)

        x_in = torch.cat([x, y_t, t_emb, cam_emb], dim=-1)  # (B,F,J,2+3+t_dim+cam_dim)
