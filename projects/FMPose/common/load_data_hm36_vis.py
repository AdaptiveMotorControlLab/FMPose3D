...
from common.generator_vis import ChunkedGenerator
...
class Fusion(data.Dataset):
    def __init__(self, opt, dataset, root_path, train=True):
        ...
        self.pad = opt.pad
        if self.train:
            self.keypoints, self.keypoints_GT, self.keypoints_no = self.prepare_data(dataset, self.train_list)
            self.cameras_train, self.poses_train, self.poses_train_2d, self.poses_train_2d_GT, self.poses_train_2d_no = \
            ...
            self.generator = ChunkedGenerator(opt.batch_size // opt.stride, self.cameras_train, self.poses_train,
                                              self.poses_train_2d, self.poses_train_2d_GT, self.poses_train_2d_no, 
            ...
            self.keypoints, self.keypoints_GT, self.keypoints_no = self.prepare_data(dataset, self.test_list)
            self.cameras_test, self.poses_test, self.poses_test_2d, self.poses_test_2d_GT, self.poses_test_2d_no = \
            ...
            self.generator = ChunkedGenerator(opt.batch_size // opt.stride, self.cameras_test, self.poses_test,
                                              self.poses_test_2d, self.poses_test_2d_GT, self.poses_test_2d_no, 
    ...
    def prepare_data(self, dataset, folder_list):
        ...
        keypoints_no = keypoints_GT
        # keypoints_no = keypoints
        ...
        keypoints_no = keypoints_no['positions_2d'].item()
            ...
            for action in dataset[subject].keys():
                ...
                for cam_idx in range(len(keypoints[subject][action])):
                    ...
                    if keypoints[subject][action][cam_idx].shape[0] > mocap_length:
                        ...
                        keypoints_no[subject][action][cam_idx] = keypoints_no[subject][action][cam_idx][:mocap_length]
        ...
        for subject in keypoints_GT.keys():
            for action in keypoints_GT[subject]:
                for cam_idx, item in enumerate(keypoints_GT[subject][action]):
                    ...
                    kps_no = keypoints_no[subject][action][cam_idx]
                    ...
                    keypoints_no[subject][action][cam_idx] = kps_no


        return keypoints, keypoints_GT, keypoints_no
    ...
    def fetch(self, dataset, subjects, subset=1, parse_3d_poses=True):
        ...
        if subset < 1:
            for key in out_poses_2d.keys():
                ...
                out_poses_2d_no[key] = out_poses_2d_no[key][start:start + n_frames:stride]
            ...
            for key in out_poses_2d.keys():
                ...
                out_poses_2d_no[key] = out_poses_2d_no[key][::stride]


                if out_poses_3d is not None:
                    out_poses_3d[key] = out_poses_3d[key][::stride]

        return out_camera_params, out_poses_3d, out_poses_2d, out_poses_2d_GT, out_poses_2d_no
    ...
    def __getitem__(self, index):
        ...
        cam, gt_3D, input_2D, input_2D_GT, input_2D_no, action, subject, cam_ind, index_image = self.generator.get_batch(seq_name, start_3d, end_3d, flip, reverse)
        
        if self.train == False and self.test_aug:
            _, _, input_2D_aug, input_2D_aug_GT, input_2D_aug_no, _, _, _, _ = self.generator.get_batch(seq_name, start_3d, end_3d, flip=True, reverse=reverse)
            input_2D = np.concatenate((np.expand_dims(input_2D,axis=0),np.expand_dims(input_2D_aug,axis=0)),0)
            input_2D_GT = np.concatenate((np.expand_dims(input_2D_GT,axis=0),np.expand_dims(input_2D_aug_GT,axis=0)),0)
            input_2D_no = np.concatenate((np.expand_dims(input_2D_no,axis=0),np.expand_dims(input_2D_aug_no,axis=0)),0)
            
        bb_box = np.array([0, 0, 1, 1])
        input_2D_update = input_2D
        input_2D_update_GT = input_2D_GT
        input_2D_update_no = input_2D_no

        # scale = np.float(1.0)

        return cam, gt_3D, input_2D_update, input_2D_update_GT, input_2D_update_no, action, subject, cam_ind, index_image
