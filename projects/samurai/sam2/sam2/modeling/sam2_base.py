    ...
    def _forward_sam_heads(
        ...
        if multimask_output and self.samurai_mode:
            if self.kf_mean is None and self.kf_covariance is None or self.stable_frames == 0:
                ...
                print("best_iou_inds:", best_iou_inds)
                print("ious.shape:", ious[0])
                print("ious:", ious[0][best_iou_inds])
                print(torch.all(ious[0][best_iou_inds]))
                print("stable_ious_threshold:", self.stable_ious_threshold)
                #  if ious[0][best_iou_inds] > self.stable_ious_threshold:
                if torch.mean(ious[0][best_iou_inds]) > self.stable_ious_threshold:
                ...
                # Get the index of the best IoU for the first batch element (index 0)
                best_iou_inds = torch.argmax(weighted_ious[0], dim=-1)
                # Convert tensor to integer for indexing
                best_iou_idx = best_iou_inds.item()
                ...
                if torch.mean(ious[0][best_iou_inds]) < self.stable_ious_threshold:
                    ...
                    # Use the integer index here
                    self.kf_mean, self.kf_covariance = self.kf.update(
                        self.kf_mean, 
                        self.kf_covariance, 
                        self.kf.xyxy_to_xyah(high_res_multibboxes[best_iou_idx])
                    )
            ...
            if self.samurai_mode:
                ...
                if frame_idx > 1:  # Ensure we have previous frames to evaluate
                    for i in range(frame_idx - 1, 1, -1):  # Iterate backwards through previous frames
                        ...
                        # Fix: Handle tensors properly by taking mean if multiple elements exist
                        iou_score_val = iou_score.mean().item() if iou_score.numel() > 1 else iou_score.item()
                        obj_score_val = obj_score.mean().item() if obj_score.numel() > 1 else obj_score.item()
                        kf_score_val = kf_score.mean().item() if kf_score is not None and kf_score.numel() > 1 else (kf_score.item() if kf_score is not None else None)
                        ...
                        if iou_score_val > self.memory_bank_iou_threshold and \
                           obj_score_val > self.memory_bank_obj_score_threshold and \
                           (kf_score_val is None or kf_score_val > self.memory_bank_kf_score_threshold):
