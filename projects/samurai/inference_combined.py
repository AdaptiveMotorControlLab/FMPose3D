...
def initialize_models(pose_config_path, pose_snapshot_path, device="cuda"):
    """Initialize both SAMURAI and pose estimation models"""
    # Initialize SAMURAI
    ...
    pose_runner, _ = get_inference_runners(
        ...
        detector_path=None,  # No detector needed
    )
    logger.info("Pose estimation model initialized")
    
    return samurai_predictor, pose_runner
...
def process_video_with_tracking(video_path, bbox_path, output_path, pose_config, pose_snapshot, device="cuda"):
    """Main processing function for video tracking and pose estimation"""
    # Step 1: Initialize models
    samurai_predictor, pose_runner = initialize_models(pose_config, pose_snapshot, device)
