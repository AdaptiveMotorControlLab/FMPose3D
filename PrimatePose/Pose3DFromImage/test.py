...
def copy_source_files(test_dir):
    """
    Copy necessary source files to test directory for reproducibility
    (aligned with Pose3D saving behavior)
    """
        ...
        for batch_idx, batch in enumerate(tqdm(test_loader, desc="Testing")):
            ...
            if batch_idx == 0 and num_vis_samples > 0:
                ...
                for i in range(batch_size):
                    ...
                    axes[0].set_ylim(256, 0)  # Flip Y axis for image coordinates
                    ...
                    axes[1].set_title('Reprojected 2D Pose (from 3D)')
                    axes[1].set_xlim(0, 256)
                    axes[1].set_ylim(256, 0)  # Flip Y axis for image coordinates
                    
                    # Plot reprojected keypoints with indices
                    ...
                    axes[2].set_title('Overlay: GT (red) vs Reprojected from Camera 3D (blue)')
                    axes[2].set_xlim(0, 256)
                    axes[2].set_ylim(256, 0)  # Flip Y axis for image coordinates
                    
                    # Plot both poses on same image
                    for j, (x_gt, y_gt, vis) in enumerate(pose_2d_np):
                        if valid_mask[i, j]:
                            axes[2].plot(x_gt, y_gt, 'ro', markersize=4, alpha=0.7, label='GT' if j == 0 else "")
                    
                    for j, (x_proj, y_proj) in enumerate(pose_2d_proj_np):
                        if valid_mask[i, j]:
                            axes[2].plot(x_proj, y_proj, 'bo', markersize=4, alpha=0.7, label='Reprojected' if j == 0 else "")
                            
                            # Draw line connecting GT and reprojected points
                            x_gt, y_gt = pose_2d_np[j, 0], pose_2d_np[j, 1]
                            axes[2].plot([x_gt, x_proj], [y_gt, y_proj], 'g-', alpha=0.3, linewidth=1)
