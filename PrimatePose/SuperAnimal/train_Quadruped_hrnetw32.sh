...
if [ "$debug" -eq 1 ]; then
    ...
    echo "Copying train_Quadruped_hrnetw32.sh and train.py to ${experiment_dir}"
    cp "$0" "$experiment_dir/"
    cp "${project_root}/train.py" "$experiment_dir/"

    # Copy train.py to the experiment folder
    echo "Copying create_coco_project_Quadruped_hrnetw32.sh and make_config.py to ${experiment_dir}"
    cp "${project_root}/create_coco_project_Quadruped_hrnetw32.sh" "$experiment_dir/"
