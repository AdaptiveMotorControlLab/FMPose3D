...
def plot_area_score(area_scores_before, area_scores_after, output_folder_path):
    ...
    output_path = os.path.join(
        output_folder_path, f"area_score_comparison_{timestamp}.png"
    )
    fig.savefig(
        output_path, dpi=300, bbox_inches="tight", pad_inches=0.05, transparent=False
    )
...
def plot_jitter_score(
    jitter_before_scores, jitter_after_scores, dataset_name, output_folder_path
):
    ...
    for score in jitter_before_scores:
        data_for_plot.append(
            {"Dataset": dataset_name, "Time": "Before", "Jitter Score": score}
        )

    # Add after adaptation data
    for score in jitter_after_scores:
        data_for_plot.append(
            {"Dataset": dataset_name, "Time": "After", "Jitter Score": score}
        )
    ...
    os.makedirs(output_folder_path, exist_ok=True)
    output_path = os.path.join(
        output_folder_path, f"jitter_score_comparison_{timestamp}.png"
    )
...
if __name__ == "__main__":
    ...
    # folder_path = "/home/ti_wang/Ti_workspace/PrimatePose/SuperAnimal/video_inference/test_video_data/pseudo_3987730-hd_1920_1080_24fps"
    
    folder_path = "/home/ti_wang/Ti_workspace/PrimatePose/SuperAnimal/video_inference/test_video_data/VA_customed_models_max_individuals_1_20250620_1730_max_individuals_1_detector_5"
    
    before_adaption_json_path = glob.glob(
        os.path.join(folder_path, "*_before_adapt.json")
    )[0]
    after_adaption_json_path = glob.glob(
        os.path.join(folder_path, "*_after_adapt.json")
    )[0]

    print("before_adaption_json_path:", before_adaption_json_path)
    ...
    if flag_calculate_jitter_score:
        ...
        postfix = "customed_models_ssdlite_rtmpose_max_individuals_1_detectors_5"
        
        # Create boxplot visualization
        image_folder_path = os.path.join(f"./figures/{postfix}")
        plot_jitter_score(
            jitter_before_scores,
            jitter_after_scores,
            dataset_name="Custom Dataset",
            output_folder_path=image_folder_path,
        )
    ...
    if flag_calculate_area_score:
        ...
        area_score_image_folder_path = os.path.join(f"./figures/{postfix}")
        plot_area_score(
            area_before_scores,
            area_after_scores,
            output_folder_path=area_score_image_folder_path,
        )
