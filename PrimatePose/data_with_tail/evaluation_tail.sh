...
# here we just set train_file to the test json file, but we will not use it;
train_file=${data_path_prefix}/primate_data/samples/with_tail/${dataset_file}_test_with_tails.json
test_file=${data_path_prefix}/primate_data/samples/with_tail/${dataset_file}_test_with_tails.json
...
# no detector
python evaluation_tail.py --project_root $data_root --pytorch_config_path $pytorch_config_path --snapshot_path $snapshot_path --train_file $train_file --test_file $test_file
