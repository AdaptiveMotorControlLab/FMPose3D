    ...
    if "keypoints" in annotation:
        ...
        for i, (x_kp, y_kp, v) in enumerate(keypoints):
            if v > 0:
                ...
                y_text = int(y_kp) - int(5 * scale_factor)
                x_text = int(x_kp) - int(5 * scale_factor)
                ...
                for (existing_x, existing_y) in existing_text_positions:
                    if abs(x_text - existing_x) < 20 and abs(y_text - existing_y) < 10:
                        y_text += int(10 * scale_factor)  # Move text slightly downward if overlap detected
                # Record this position
                existing_text_positions.append((x_text, y_text))
                
                thickness_dict = {"oap": 2,   "oms": 1,   "omc": 1, "mp": 1,     "ap10k": 1,   "aptv2": 1,   "lote": 1,   "ak": 1,   "deepwild": 1,   "chimpact": 1,   "mit": 2,   "riken": 1,    "pfm": 2}
                fontScale_dict = {"oap": 1.6, "oms": 0.9, "omc": 0.9, "mp": 1.3, "ap10k": 0.8, "aptv2": 1.1, "lote": 0.8, "ak": 1.0, "deepwild": 0.6, "chimpact": 0.6, "mit": 1.2, "riken": 0.45,  "pfm": 1.2}
                ...
                cv2.putText(
                    ...
                    org=(int(x_kp*upscale_factor), y_text*upscale_factor),
...
def main():
    ...
    mode = "test"
    # annotation_file_path = f"/home/ti_wang/Ti_workspace/PrimatePose/data/tiwang/primate_data/splitted_{mode}_datasets/{dataset_name}_{mode}.json"
    annotation_file_path = f"/home/ti_wang/Ti_workspace/PrimatePose/data/tiwang/primate_data/analyse_train_datasets/riken_test_pose.json"
    with open(annotation_file_path, "r") as f:
        annotation_file = json.load(f)
