...
def main(
    ...
    for split, filepath in [("train", train_file), ("test", test_file)]:
        ...
        images = []
        data = []
        print(f"Processing {len(coco_data['images'])} images for {split} set...")
        for img in tqdm(coco_data["images"], desc=f"Creating symlinks for {split}", unit="img"):
            ...
            if os.path.exists(dest_filepath):
                os.remove(dest_filepath)  # Remove if already exists
            os.symlink(os.path.abspath(source_filepath), dest_filepath)
