...
class CameraPredictor(nn.Module):
    ...
    def forward(self, visual_features):
        ...
        focal_length = torch.sigmoid(camera_params[:, 0:1]) * 1000 + 500  # Range [500, 1500]
...
class Pose3DPredictor(nn.Module):
    """
    Predicts 3D pose from image features only
    """
    ...
    def forward(self, visual_features):
        ...
        """
        # Predict 3D pose using only visual features
        pose_3d_flat = self.predictor(visual_features)
...
class Pose3DEstimator(nn.Module):
    ...
    def forward(self, image, pose_2d, valid_mask=None):
        ...
        # Predict 3D pose
        pose_3d = self.pose_3d_predictor(visual_features)
