...
def save_checkpoint(model, optimizer, scheduler, epoch, best_loss, save_path, best_epoch=None):
    ...
    checkpoint = {
        ...
        'best_loss': best_loss,
        'best_epoch': best_epoch,  # Record the epoch of best model
    }
    torch.save(checkpoint, save_path)


def manage_checkpoints(checkpoint_dir, max_checkpoints=3, logger=None):
    """
    Manage checkpoint files, keeping only the most recent max_checkpoints files
    Args:
    ...
    # Find all checkpoint files (not including best_model.pth and final_model.pth)
    ...
    for file_path in checkpoint_files:
        ...
        if match:
            ...
            checkpoints_with_epochs.append((epoch, file_path))
    ...
    files_to_keep = checkpoints_with_epochs[:max_checkpoints]
    files_to_delete = checkpoints_with_epochs[max_checkpoints:]
    
    # Delete old checkpoint files
    for epoch, file_path in files_to_delete:
        try:
            os.remove(file_path)
            if logger:
                logger.info(f"Deleted old checkpoint: {os.path.basename(file_path)} (epoch {epoch})")
        except OSError as e:
            if logger:
                logger.warning(f"Failed to delete checkpoint {file_path}: {e}")
    
    if logger and files_to_delete:
        kept_epochs = [epoch for epoch, _ in files_to_keep]
        logger.info(f"Keeping {len(files_to_keep)} most recent checkpoints (epochs: {kept_epochs})")
...
def main():
    ...
    parser.add_argument('--max_checkpoints', type=int, default=3,
                       help='Maximum number of checkpoint files to keep (older ones will be deleted automatically)')
    ...
    best_epoch = -1  # Track the epoch of best model
    ...
    if args.resume:
        ...
        start_epoch += 1
        # Try to load best epoch from checkpoint if available
        checkpoint = torch.load(args.resume, map_location='cpu')
        best_epoch = checkpoint.get('best_epoch', start_epoch - 1)
    
    # Training loop
    logger.info("Starting training...")
    logger.info(f"Checkpoint management: saving every {args.save_freq} epochs, keeping max {args.max_checkpoints} files")
    for epoch in range(start_epoch, args.epochs):
        ...
        if epoch % args.val_freq == 0:
            ...
            if val_loss < best_val_loss:
                ...
                best_epoch = epoch  # Record the epoch of best model
                best_model_path = os.path.join(args.output_dir, 'best_model.pth')
                save_checkpoint(model, optimizer, scheduler, epoch, best_val_loss, best_model_path, best_epoch)
                logger.info(f"New best model saved with validation loss: {best_val_loss:.4f} at epoch {epoch}")
        
        # Save checkpoint
        if epoch % args.save_freq == 0:
            checkpoint_path = os.path.join(args.output_dir, f'checkpoint_epoch_{epoch}.pth')
            save_checkpoint(model, optimizer, scheduler, epoch, best_val_loss, checkpoint_path, best_epoch)
            logger.info(f"Checkpoint saved: {checkpoint_path}")
            
            # Manage checkpoint files (keep only the most recent ones)
            manage_checkpoints(args.output_dir, max_checkpoints=args.max_checkpoints, logger=logger)
    
    # Save final model
    final_model_path = os.path.join(args.output_dir, 'final_model.pth')
    save_checkpoint(model, optimizer, scheduler, args.epochs - 1, best_val_loss, final_model_path, best_epoch)
    logger.info(f"Final model saved: {final_model_path}")
    logger.info(f"Training summary: Best validation loss {best_val_loss:.4f} achieved at epoch {best_epoch}")
    
    # Close tensorboard writer
    writer.close()
    logger.info("Training completed!")
    
    # Information about experiment directory structure
    logger.info("📁 Experiment files generated:")
    logger.info(f"  🏆 Best model: {os.path.join(args.output_dir, 'best_model.pth')}")
    logger.info(f"  🔚 Final model: {os.path.join(args.output_dir, 'final_model.pth')}")
    logger.info(f"  📋 Source code: {os.path.join(args.output_dir, 'source_code/')}")
    logger.info(f"  📝 Training args: {os.path.join(args.output_dir, 'training_args.txt')}")
    logger.info(f"  📊 TensorBoard: {os.path.join(args.output_dir, 'tensorboard/')}")
